...
describe('parser', function () {
  ...
  it('parses handlebars files', function (done) {
    let result
    const i18nextParser = new i18nTransform()
    const fakeFile = new Vinyl({
  ...
  it('parses javascript files', function (done) {
    let result
    const i18nextParser = new i18nTransform()
    const fakeFile = new Vinyl({
      contents: fs.readFileSync( path.resolve(__dirname, 'templating/javascript.js') ),
      path: 'file.js'
    })
    const expected = {
      first: '',
  ...
  it('retrieves values in existing catalog', function (done) {
    let result
  ...
  it('does not leak values between locales', function (done) {
    let resultEN
    let resultFR
    const i18nextParser = new i18nTransform({output: 'test/locales'})
    const fakeFile = new Vinyl({
  ...
  it('retrieves context values in existing catalog', function (done) {
    let result
    const i18nextParser = new i18nTransform({output: 'test/locales'})
    const fakeFile = new Vinyl({
  ...
  it('retrieves plural values in existing catalog', function (done) {
    let result
    const i18nextParser = new i18nTransform({output: 'test/locales'})
    const fakeFile = new Vinyl({
  ...
  it('retrieves plural and context values in existing catalog', function (done) {
    let result
    const i18nextParser = new i18nTransform({output: 'test/locales'})
    const fakeFile = new Vinyl({
  ...
  describe('options', function () {
    it('handles filename and extension with $LOCALE and $NAMESPACE var', function (done) {
      let results = []
      const i18nextParser = new i18nTransform({
        locales: ['en'],
        defaultNamespace: 'default',
      ...
      i18nextParser.on('end', function () {
        const expectedFiles = [
          'en/p-en-default.en.i18n', 'en/p-en-default_old.en.i18n'
        ]
        let length = expectedFiles.length
    ...
    it('handles skipping the old catalog with createOldLibraries=false', function (done) {
      let results = []
      const i18nextParser = new i18nTransform({
        locales: ['en', 'de', 'fr'],
        defaultNamespace: 'default',
    ...
    describe('lexers', function () {
      it('support custom lexers options', function (done) {
        let result
        const i18nextParser = new i18nTransform({
          lexers: {
            js: [{
    ...
    describe('sort', function () {
      it('does not sort by default', function (done) {
        let result
        const i18nextParser = new i18nTransform()
        const fakeFile = new Vinyl({
          contents: Buffer.from("asd t('ccc') t('aaa') t('bbb.bbb') t('bbb.aaa')"),
          path: 'file.js'
        })
      ...
      it('supports sort as an option', function (done) {
        let result
        const i18nextParser = new i18nTransform({
          sort: true
        })
  ...
  describe('events', function () {
    it('emits a `reading` event', function (done) {
      let result
      const i18nextParser = new i18nTransform()
      const fakeFile = new Vinyl({
      ...
      i18nextParser.on('finish', function () {
    ...
    it('emits a `error` event if the catalog is not valid json', function (done) {
      const i18nextParser = new i18nTransform({output: 'test/locales'})
      const fakeFile = new Vinyl({
    ...
    it('emits an `error` if a lexer does not exist', function (done) {
      const results = []
    ...
    it('emits a `warning` event if a key contains a variable', function (done) {
      const i18nextParser = new i18nTransform({output: 'test/locales'})
      const fakeFile = new Vinyl({
